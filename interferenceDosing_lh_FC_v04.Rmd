---
title: "interferenceDosing_lh_FC_v04"
...
## Data import
```{r Load in FC data}
setwd("/Volumes/mnl/Data/Adaptation/interference_dosing/Post_Step_3_FC") #Mac
#setwd("Z:\\Data\\Adaptation\\interference_dosing\\Post_Step_3_FC") #PC
...
colnames(fcData.fs) = c('subID', 'wrongTrial',sample)
fcData.fs$trial = trial
fcData.fs = arrange.vars(fcData.fs,vars = c("trial"=3))

factors = c('subID', 'wrongTrial')
fcData.fs[,factors] = lapply(fcData.fs[,factors], factor)

# Add in a column for group number
fcData.fs$group = rep(0,48*numSub)
for (i in 1:(nrow(fcData.fs))){
  fcData.fs$group[i] = substr(as.character(fcData.fs$subID[i]),1,1)
}
fcData.fs = arrange.vars(fcData.fs, vars = c("group" = 4))
fcData.fs$group = as.factor(fcData.fs$group)
...
sample = seq(1,1000)
trial = rep(1:48,numSub)
colnames(fcData.cmd) = c('subID','wrongTrial',sample)
fcData.cmd$trial = trial
fcData.cmd = arrange.vars(fcData.cmd,vars = c("trial"=3))

factors = c('subID', 'wrongTrial')
fcData.cmd[,factors] = lapply(fcData.cmd[,factors], factor)

# Add in a column for group number
fcData.cmd$group = rep(0,48*numSub)
for (i in 1:(nrow(fcData.cmd))){
  fcData.cmd$group[i] = substr(as.character(fcData.cmd$subID[i]),1,1)
}
fcData.cmd = arrange.vars(fcData.cmd, vars = c("group" = 4))
fcData.cmd$group = as.factor(fcData.cmd$group)
```
...
temp = fcData.fs %>% select(-c(wrongTrial)) %>% filter(trial %in% 7:12) %>% group_by(subID) %>% summarise_all(funs(mean(.,na.rm = TRUE)))
temp2 = data.frame(matrix(ncol = 1000, nrow = 48*numSub))
for (i in 1:numSub){
  for (j in 1:48){
    temp2[(48*(i-1)+j),] = fcData.fs[(48*(i-1)+j),5:1004] - temp[i,4:1003] # This works, but takes a while...be patient :)
  }
}
fcData.fs = cbind(fcData.fs[,1:4],temp2)
rm(temp, temp2)
colnames(fcData.fs)[5:1004] = c(1:1000)
...
```{r Group means and group sem}
# Need to subtract off the mean of kin basline from all trials, by subject
temp = fcData.cmd %>% select(-c(wrongTrial)) %>% filter(trial %in% 7:12) %>% group_by(subID) %>% summarise_all(funs(mean(.,na.rm = TRUE)))
temp2 = data.frame(matrix(ncol = 1000, nrow = 48*numSub))
for (i in 1:numSub){
  for (j in 1:48){
    temp2[(48*(i-1)+j),] = fcData.cmd[(48*(i-1)+j),5:1004] - temp[i,4:1003] # This works, but takes a while...be patient :)
  }
}
fcData.cmd = cbind(fcData.cmd[,1:4],temp2)
rm(temp, temp2)
colnames(fcData.cmd)[5:1004] = c(1:1000)
