...
if strcmp(str,'MACI64') == 1
    ...
    cd('Z:\Data\Adaptation\interference_dosing\Post_Step_1\'); % Lab PCs
    %cd('C:\Users\Alex\Desktop\IFDosing\Post_Step_1'); % Home PC
    fname = uigetfile('*lh.mat');
    fname = fname(1:22);
    fcTrials = xlsread('Z:Data\Adaptation\interference_dosing\Force_Channel_Trials_12_03_16.xlsx','Sheet1'); %Lab PCs
...
% transpose and calculate standardized variable
mov_int_c = mov_int_c';
bkup_mean = nanmean(mov_int_c(upTrials(11:20)));
bkup_std = nanstd(mov_int_c(upTrials(11:20)));
mov_int_up_st = (mov_int_c(upTrials) - bkup_mean)/bkup_std;

bkdown_mean = nanmean(mov_int_c(downTrials(11:20)));
bkdown_std = nanstd(mov_int_c(downTrials(11:20)));
...
axis([0 20 0 20]); set(gca,'LineWidth',2,'XTick',[1 10 20],'YTick',0:4:20,'YTickLabel', 0:4:20,'FontName','Arial','FontSize',10); ylabel('mov_int [s]'); title('vis-pre','fontsize',11);
...
bkup_mean = nanmean(norm_jerk_c(upTrials(11:20)));
bkup_std = nanstd(norm_jerk_c(upTrials(11:20)));
norm_jerk_up_st = (norm_jerk_c(upTrials) - bkup_mean)/bkup_std;

bkdown_mean = nanmean(norm_jerk_c(downTrials(11:20)));
bkdown_std = nanstd(norm_jerk_c(downTrials(11:20)));
...
axis([0 20 0 1000]); set(gca,'LineWidth',2,'XTick',[1 10 20],'YTick',0:100:1000,'YTickLabel', 0:100:1000,'FontName','Arial','FontSize',10); ylabel('norm_jerk [s]'); title('vis-pre','fontsize',11);
...
% transpose and calculate standardized variable
velPeak_c = velPeak_c';
bkup_mean = nanmean(velPeak_c(upTrials(11:20)));
bkup_std = nanstd(velPeak_c(upTrials(11:20)));
velPeak_up_st = (velPeak_c(upTrials) - bkup_mean)/bkup_std;

bkdown_mean = nanmean(velPeak_c(downTrials(11:20)));
bkdown_std = nanstd(velPeak_c(downTrials(11:20)));
...
axis([0 20 0 100]); set(gca,'LineWidth',2,'XTick',[1 10 20],'YTick',0:10:100,'YTickLabel', 0:10:100,'FontName','Arial','FontSize',10); ylabel('velPeak [s]'); title('vis-pre','fontsize',11);
...
%% Lateral Force at OFFSET using Force at handles
for i = 1:numTrials
    if wrong_trial(i) == 0 && channel_trial(i) == 0
        LFoff_FS(i) = sortData(i).Left_FS_ForceX(offset(i));
    else
        LFoff_FS(i) = NaN;
    end
end
LFoff_FS(wrong_trial == 1) = NaN;
LFoff_FS(channel_trial == 1) = NaN;

data = outlier_t(LFoff_FS(upTrials(1:10))); % Outlier for visual baseline
LFoff_FS_c(upTrials(1:10)) = data;
data = outlier_t(LFoff_FS(downTrials(1:10)));
LFoff_FS_c(downTrials(1:10)) = data;
clear data;

data = outlier_t(LFoff_FS(upTrials(11:20))); % Outlier for kin baseline
LFoff_FS_c(upTrials(11:20)) = data;
data = outlier_t(LFoff_FS(downTrials(11:20)));
LFoff_FS_c(downTrials(11:20)) = data;
clear data;

LFoff_FS_c(upTrials(21)) = NaN; LFoff_FS_c(downTrials(21)) = NaN; % Toss out catch trials

% data = outlier_t(LFoff_FS(upTrials(14:23))); % Outlier for first 20 exposure
% LFoff_FS_c(upTrials(14:23)) = data;
% data = outlier_t(LFoff_FS(downTrials(14:23)));
% LFoff_FS_c(downTrials(14:23)) = data;
% clear data;
LFoff_FS_c(upTrials(22:31)) = LFoff_FS(upTrials(22:31));
LFoff_FS_c(downTrials(22:31)) = LFoff_FS(downTrials(22:31));

data = outlier_t(LFoff_FS(upTrials(32:91))); % Outlier for last 100 exposure
LFoff_FS_c(upTrials(32:91)) = data;
data = outlier_t(LFoff_FS(downTrials(32:91)));
LFoff_FS_c(downTrials(32:91)) = data;
clear data;

data = outlier_t(LFoff_FS(upTrials(92:96))); % Outlier for first 10 post-exp
LFoff_FS_c(upTrials(92:96)) = data;
data = outlier_t(LFoff_FS(downTrials(92:96)));
LFoff_FS_c(downTrials(92:96)) = data;
clear data;

data = outlier_t(LFoff_FS(upTrials(97:111))); % Outlier for last 10 post-exp
LFoff_FS_c(upTrials(97:111)) = data;
data = outlier_t(LFoff_FS(downTrials(97:111)));
LFoff_FS_c(downTrials(97:111)) = data;
clear data;

% transpose and calculate standardized variable
LFoff_FS_c = LFoff_FS_c';
bkup_mean = nanmean(LFoff_FS_c(upTrials(11:20)));
bkup_std = nanstd(LFoff_FS_c(upTrials(11:20)));
LFoff_FS_up_st = (LFoff_FS_c(upTrials) - bkup_mean)/bkup_std;

bkdown_mean = nanmean(LFoff_FS_c(downTrials(11:20)));
bkdown_std = nanstd(LFoff_FS_c(downTrials(11:20)));
LFoff_FS_down_st = (LFoff_FS_c(downTrials) - bkdown_mean)/bkdown_std;

clear bkup_mean; clear bkup_std; clear bkdown_mean; clear bkdown_std;
LFoff_FS = LFoff_FS';

%% Plotting Code for LFoff_FS
figure
set(gcf,'Color','w','Position',[560 528 600 420])
hold on;

subplot('Position',[0.06 0.2 0.1 0.6]); hold on;
plot(upTrials(1:10),LFoff_FS(upTrials(1:10)),'bo');
hold on
plot(upTrials(1:10),LFoff_FS_c(upTrials(1:10)),'bx');
hold on
plot(downTrials(1:10),LFoff_FS(downTrials(1:10)),'ro');
hold on
plot(downTrials(1:10),LFoff_FS_c(downTrials(1:10)),'rx');
axis([0 20 -3 3]); set(gca,'LineWidth',2,'XTick',[1 10 20],'YTick',-3:0.5:3,'YTickLabel', -3:0.5:3,'FontName','Arial','FontSize',10); ylabel('LFoff_FS [N]'); title('vis-pre','fontsize',11);

hold on
subplot('Position',[0.19 0.2 0.1 0.6]); hold on;
plot(upTrials(11:20)-kbTrials(1)+1,LFoff_FS(upTrials(11:20)),'bo');
hold on
plot(upTrials(11:20)-kbTrials(1)+1,LFoff_FS_c(upTrials(11:20)),'bx');
hold on
plot(downTrials(11:20)-kbTrials(1)+1,LFoff_FS(downTrials(11:20)),'ro');
hold on
plot(downTrials(11:20)-kbTrials(1)+1,LFoff_FS_c(downTrials(11:20)),'rx');
axis([0 20 -3 3]); set(gca,'LineWidth',2,'XTick',[1 10 20],'YTick',[],'YTickLabel',[],'FontName','Arial','FontSize',10); title('kin-pre','fontsize',11);

hold on
subplot('Position',[0.32 0.2 0.4 0.6]); hold on;
plot(upTrials(22:91)-exTrials(1)+1,LFoff_FS(upTrials(22:91)),'bo');
hold on
plot(upTrials(22:91)-exTrials(1)+1,LFoff_FS_c(upTrials(22:91)),'bx');
hold on
plot(downTrials(22:91)-exTrials(1)+1,LFoff_FS(downTrials(22:91)),'ro');
hold on
plot(downTrials(22:91)-exTrials(1)+1,LFoff_FS_c(downTrials(22:91)),'rx');
axis([0 140 -3 3]); set(gca,'LineWidth',2,'XTick',[1 20 70 120 140],'YTick',[],'YTickLabel',[],'FontName','Arial','FontSize',10); title(['exposure'], 'fontsize',11); xlabel('Trials','fontsize',11);

hold on
subplot('Position',[0.75 0.2 0.24 0.6]); hold on;
plot(upTrials(92:111)-peTrials(1)+1,LFoff_FS(upTrials(92:111)),'bo');
hold on
plot(upTrials(92:111)-peTrials(1)+1,LFoff_FS_c(upTrials(92:111)),'bx');
hold on
plot(downTrials(92:111)-peTrials(1)+1,LFoff_FS(downTrials(92:111)),'ro');
hold on
plot(downTrials(92:111)-peTrials(1)+1,LFoff_FS_c(downTrials(92:111)),'rx');
axis([0 40 -3 3]); set(gca,'LineWidth',2,'XTick',[1 20 40],'YTick',[],'YTickLabel',[],'FontName','Arial','FontSize',10); title(['post-exposure'], 'fontsize',11); xlabel('Trials','fontsize',11);
title([subID(7:9), ' ', 'LFoff_FS'])

%% Lateral Force at OFFSET using Force command
for i = 1:numTrials
    if wrong_trial(i) == 0 && channel_trial(i) == 0
        LFoff_CMD(i) = sortData(i).Left_Hand_ForceCMD_X(offset(i));
    else
        LFoff_CMD(i) = NaN;
    end
end
LFoff_CMD(wrong_trial == 1) = NaN;
LFoff_CMD(channel_trial == 1) = NaN;

data = outlier_t(LFoff_CMD(upTrials(1:10))); % Outlier for visual baseline
LFoff_CMD_c(upTrials(1:10)) = data;
data = outlier_t(LFoff_CMD(downTrials(1:10)));
LFoff_CMD_c(downTrials(1:10)) = data;
clear data;

data = outlier_t(LFoff_CMD(upTrials(11:20))); % Outlier for kin baseline
LFoff_CMD_c(upTrials(11:20)) = data;
data = outlier_t(LFoff_CMD(downTrials(11:20)));
LFoff_CMD_c(downTrials(11:20)) = data;
clear data;

LFoff_CMD_c(upTrials(21)) = NaN; LFoff_CMD_c(downTrials(21)) = NaN; % Toss out catch trials

% data = outlier_t(LFoff_CMD(upTrials(14:23))); % Outlier for first 20 exposure
% LFoff_CMD_c(upTrials(14:23)) = data;
% data = outlier_t(LFoff_CMD(downTrials(14:23)));
% LFoff_CMD_c(downTrials(14:23)) = data;
% clear data;
LFoff_CMD_c(upTrials(22:31)) = LFoff_CMD(upTrials(22:31));
LFoff_CMD_c(downTrials(22:31)) = LFoff_CMD(downTrials(22:31));

data = outlier_t(LFoff_CMD(upTrials(32:91))); % Outlier for last 100 exposure
LFoff_CMD_c(upTrials(32:91)) = data;
data = outlier_t(LFoff_CMD(downTrials(32:91)));
LFoff_CMD_c(downTrials(32:91)) = data;
clear data;

data = outlier_t(LFoff_CMD(upTrials(92:96))); % Outlier for first 10 post-exp
LFoff_CMD_c(upTrials(92:96)) = data;
data = outlier_t(LFoff_CMD(downTrials(92:96)));
LFoff_CMD_c(downTrials(92:96)) = data;
clear data;

data = outlier_t(LFoff_CMD(upTrials(97:111))); % Outlier for last 10 post-exp
LFoff_CMD_c(upTrials(97:111)) = data;
data = outlier_t(LFoff_CMD(downTrials(97:111)));
LFoff_CMD_c(downTrials(97:111)) = data;
clear data;

% transpose and calculate standardized variable
LFoff_CMD_c = LFoff_CMD_c';
bkup_mean = nanmean(LFoff_CMD_c(upTrials(11:20)));
bkup_std = nanstd(LFoff_CMD_c(upTrials(11:20)));
LFoff_CMD_up_st = (LFoff_CMD_c(upTrials) - bkup_mean)/bkup_std;

bkdown_mean = nanmean(LFoff_CMD_c(downTrials(11:20)));
bkdown_std = nanstd(LFoff_CMD_c(downTrials(11:20)));
LFoff_CMD_down_st = (LFoff_CMD_c(downTrials) - bkdown_mean)/bkdown_std;

clear bkup_mean; clear bkup_std; clear bkdown_mean; clear bkdown_std;
LFoff_CMD = LFoff_CMD';

%% Plotting Code for LFoff_CMD
figure
set(gcf,'Color','w','Position',[560 528 600 420])
hold on;

subplot('Position',[0.06 0.2 0.1 0.6]); hold on;
plot(upTrials(1:10),LFoff_CMD(upTrials(1:10)),'bo');
hold on
plot(upTrials(1:10),LFoff_CMD_c(upTrials(1:10)),'bx');
hold on
plot(downTrials(1:10),LFoff_CMD(downTrials(1:10)),'ro');
hold on
plot(downTrials(1:10),LFoff_CMD_c(downTrials(1:10)),'rx');
axis([0 20 -3 3]); set(gca,'LineWidth',2,'XTick',[1 10 20],'YTick',-3:0.5:3,'YTickLabel', -3:0.5:3,'FontName','Arial','FontSize',10); ylabel('LFoff_CMD [N]'); title('vis-pre','fontsize',11);

hold on
subplot('Position',[0.19 0.2 0.1 0.6]); hold on;
plot(upTrials(11:20)-kbTrials(1)+1,LFoff_CMD(upTrials(11:20)),'bo');
hold on
plot(upTrials(11:20)-kbTrials(1)+1,LFoff_CMD_c(upTrials(11:20)),'bx');
hold on
plot(downTrials(11:20)-kbTrials(1)+1,LFoff_CMD(downTrials(11:20)),'ro');
hold on
plot(downTrials(11:20)-kbTrials(1)+1,LFoff_CMD_c(downTrials(11:20)),'rx');
axis([0 20 -3 3]); set(gca,'LineWidth',2,'XTick',[1 10 20],'YTick',[],'YTickLabel',[],'FontName','Arial','FontSize',10); title('kin-pre','fontsize',11);

hold on
subplot('Position',[0.32 0.2 0.4 0.6]); hold on;
plot(upTrials(22:91)-exTrials(1)+1,LFoff_CMD(upTrials(22:91)),'bo');
hold on
plot(upTrials(22:91)-exTrials(1)+1,LFoff_CMD_c(upTrials(22:91)),'bx');
hold on
plot(downTrials(22:91)-exTrials(1)+1,LFoff_CMD(downTrials(22:91)),'ro');
hold on
plot(downTrials(22:91)-exTrials(1)+1,LFoff_CMD_c(downTrials(22:91)),'rx');
axis([0 140 -3 3]); set(gca,'LineWidth',2,'XTick',[1 20 70 120 140],'YTick',[],'YTickLabel',[],'FontName','Arial','FontSize',10); title(['exposure'], 'fontsize',11); xlabel('Trials','fontsize',11);

hold on
subplot('Position',[0.75 0.2 0.24 0.6]); hold on;
plot(upTrials(92:111)-peTrials(1)+1,LFoff_CMD(upTrials(92:111)),'bo');
hold on
plot(upTrials(92:111)-peTrials(1)+1,LFoff_CMD_c(upTrials(92:111)),'bx');
hold on
plot(downTrials(92:111)-peTrials(1)+1,LFoff_CMD(downTrials(92:111)),'ro');
hold on
plot(downTrials(92:111)-peTrials(1)+1,LFoff_CMD_c(downTrials(92:111)),'rx');
axis([0 40 -3 3]); set(gca,'LineWidth',2,'XTick',[1 20 40],'YTick',[],'YTickLabel',[],'FontName','Arial','FontSize',10); title(['post-exposure'], 'fontsize',11); xlabel('Trials','fontsize',11);
title([subID(7:9), ' ', 'LFoff_CMD'])

%% Lateral Force at velPeak using Force sensor
for i = 1:numTrials
    if wrong_trial(i) == 0 && channel_trial(i) == 0
        LFvp_FS(i) = sortData(i).Left_FS_ForceX(indPeak(i));
    else
        LFvp_FS(i) = NaN;
    end
end
LFvp_FS(wrong_trial == 1) = NaN;
LFvp_FS(channel_trial == 1) = NaN;

data = outlier_t(LFvp_FS(upTrials(1:10))); % Outlier for visual baseline
LFvp_FS_c(upTrials(1:10)) = data;
data = outlier_t(LFvp_FS(downTrials(1:10)));
LFvp_FS_c(downTrials(1:10)) = data;
clear data;

data = outlier_t(LFvp_FS(upTrials(11:20))); % Outlier for kin baseline
LFvp_FS_c(upTrials(11:20)) = data;
data = outlier_t(LFvp_FS(downTrials(11:20)));
LFvp_FS_c(downTrials(11:20)) = data;
clear data;

LFvp_FS_c(upTrials(21)) = NaN; LFvp_FS_c(downTrials(21)) = NaN; % Toss out catch trials

% data = outlier_t(LFvp_FS(upTrials(14:23))); % Outlier for first 20 exposure
% LFvp_FS_c(upTrials(14:23)) = data;
% data = outlier_t(LFvp_FS(downTrials(14:23)));
% LFvp_FS_c(downTrials(14:23)) = data;
% clear data;
LFvp_FS_c(upTrials(22:31)) = LFvp_FS(upTrials(22:31));
LFvp_FS_c(downTrials(22:31)) = LFvp_FS(downTrials(22:31));

data = outlier_t(LFvp_FS(upTrials(32:91))); % Outlier for last 100 exposure
LFvp_FS_c(upTrials(32:91)) = data;
data = outlier_t(LFvp_FS(downTrials(32:91)));
LFvp_FS_c(downTrials(32:91)) = data;
clear data;

data = outlier_t(LFvp_FS(upTrials(92:96))); % Outlier for first 10 post-exp
LFvp_FS_c(upTrials(92:96)) = data;
data = outlier_t(LFvp_FS(downTrials(92:96)));
LFvp_FS_c(downTrials(92:96)) = data;
clear data;

data = outlier_t(LFvp_FS(upTrials(97:111))); % Outlier for last 10 post-exp
LFvp_FS_c(upTrials(97:111)) = data;
data = outlier_t(LFvp_FS(downTrials(97:111)));
LFvp_FS_c(downTrials(97:111)) = data;
clear data;

% transpose and calculate standardized variable
LFvp_FS_c = LFvp_FS_c';
bkup_mean = nanmean(LFvp_FS_c(upTrials(11:20)));
bkup_std = nanstd(LFvp_FS_c(upTrials(11:20)));
LFvp_FS_up_st = (LFvp_FS_c(upTrials) - bkup_mean)/bkup_std;

bkdown_mean = nanmean(LFvp_FS_c(downTrials(11:20)));
bkdown_std = nanstd(LFvp_FS_c(downTrials(11:20)));
LFvp_FS_down_st = (LFvp_FS_c(downTrials) - bkdown_mean)/bkdown_std;

clear bkup_mean; clear bkup_std; clear bkdown_mean; clear bkdown_std;
LFvp_FS = LFvp_FS';

%% Plotting Code for LFvp_FS
figure
set(gcf,'Color','w','Position',[560 528 600 420])
hold on;

subplot('Position',[0.06 0.2 0.1 0.6]); hold on;
plot(upTrials(1:10),LFvp_FS(upTrials(1:10)),'bo');
hold on
plot(upTrials(1:10),LFvp_FS_c(upTrials(1:10)),'bx');
hold on
plot(downTrials(1:10),LFvp_FS(downTrials(1:10)),'ro');
hold on
plot(downTrials(1:10),LFvp_FS_c(downTrials(1:10)),'rx');
axis([0 20 -3 3]); set(gca,'LineWidth',2,'XTick',[1 10 20],'YTick',-3:0.5:3,'YTickLabel', -3:0.5:3,'FontName','Arial','FontSize',10); ylabel('LFvp_FS [N]'); title('vis-pre','fontsize',11);

hold on
subplot('Position',[0.19 0.2 0.1 0.6]); hold on;
plot(upTrials(11:20)-kbTrials(1)+1,LFvp_FS(upTrials(11:20)),'bo');
hold on
plot(upTrials(11:20)-kbTrials(1)+1,LFvp_FS_c(upTrials(11:20)),'bx');
hold on
plot(downTrials(11:20)-kbTrials(1)+1,LFvp_FS(downTrials(11:20)),'ro');
hold on
plot(downTrials(11:20)-kbTrials(1)+1,LFvp_FS_c(downTrials(11:20)),'rx');
axis([0 20 -3 3]); set(gca,'LineWidth',2,'XTick',[1 10 20],'YTick',[],'YTickLabel',[],'FontName','Arial','FontSize',10); title('kin-pre','fontsize',11);

hold on
subplot('Position',[0.32 0.2 0.4 0.6]); hold on;
plot(upTrials(22:91)-exTrials(1)+1,LFvp_FS(upTrials(22:91)),'bo');
hold on
plot(upTrials(22:91)-exTrials(1)+1,LFvp_FS_c(upTrials(22:91)),'bx');
hold on
plot(downTrials(22:91)-exTrials(1)+1,LFvp_FS(downTrials(22:91)),'ro');
hold on
plot(downTrials(22:91)-exTrials(1)+1,LFvp_FS_c(downTrials(22:91)),'rx');
axis([0 140 -3 3]); set(gca,'LineWidth',2,'XTick',[1 20 70 120 140],'YTick',[],'YTickLabel',[],'FontName','Arial','FontSize',10); title(['exposure'], 'fontsize',11); xlabel('Trials','fontsize',11);

hold on
subplot('Position',[0.75 0.2 0.24 0.6]); hold on;
plot(upTrials(92:111)-peTrials(1)+1,LFvp_FS(upTrials(92:111)),'bo');
hold on
plot(upTrials(92:111)-peTrials(1)+1,LFvp_FS_c(upTrials(92:111)),'bx');
hold on
plot(downTrials(92:111)-peTrials(1)+1,LFvp_FS(downTrials(92:111)),'ro');
hold on
plot(downTrials(92:111)-peTrials(1)+1,LFvp_FS_c(downTrials(92:111)),'rx');
axis([0 40 -3 3]); set(gca,'LineWidth',2,'XTick',[1 20 40],'YTick',[],'YTickLabel',[],'FontName','Arial','FontSize',10); title(['post-exposure'], 'fontsize',11); xlabel('Trials','fontsize',11);
title([subID(7:9), ' ', 'LFvp_FS'])

%% Lateral Force at velPeak using Force command
for i = 1:numTrials
    if wrong_trial(i) == 0 && channel_trial(i) == 0
        LFvp_CMD(i) = sortData(i).Left_Hand_ForceCMD_X(indPeak(i));
    else
        LFvp_CMD(i) = NaN;
    end
end
LFvp_CMD(wrong_trial == 1) = NaN;
LFvp_CMD(channel_trial == 1) = NaN;

data = outlier_t(LFvp_CMD(upTrials(1:10))); % Outlier for visual baseline
LFvp_CMD_c(upTrials(1:10)) = data;
data = outlier_t(LFvp_CMD(downTrials(1:10)));
LFvp_CMD_c(downTrials(1:10)) = data;
clear data;

data = outlier_t(LFvp_CMD(upTrials(11:20))); % Outlier for kin baseline
LFvp_CMD_c(upTrials(11:20)) = data;
data = outlier_t(LFvp_CMD(downTrials(11:20)));
LFvp_CMD_c(downTrials(11:20)) = data;
clear data;

LFvp_CMD_c(upTrials(21)) = NaN; LFvp_CMD_c(downTrials(21)) = NaN; % Toss out catch trials

% data = outlier_t(LFvp_CMD(upTrials(14:23))); % Outlier for first 20 exposure
% LFvp_CMD_c(upTrials(14:23)) = data;
% data = outlier_t(LFvp_CMD(downTrials(14:23)));
% LFvp_CMD_c(downTrials(14:23)) = data;
% clear data;
LFvp_CMD_c(upTrials(22:31)) = LFvp_CMD(upTrials(22:31));
LFvp_CMD_c(downTrials(22:31)) = LFvp_CMD(downTrials(22:31));

data = outlier_t(LFvp_CMD(upTrials(32:91))); % Outlier for last 100 exposure
LFvp_CMD_c(upTrials(32:91)) = data;
data = outlier_t(LFvp_CMD(downTrials(32:91)));
LFvp_CMD_c(downTrials(32:91)) = data;
clear data;

data = outlier_t(LFvp_CMD(upTrials(92:96))); % Outlier for first 10 post-exp
LFvp_CMD_c(upTrials(92:96)) = data;
data = outlier_t(LFvp_CMD(downTrials(92:96)));
LFvp_CMD_c(downTrials(92:96)) = data;
clear data;

data = outlier_t(LFvp_CMD(upTrials(97:111))); % Outlier for last 10 post-exp
LFvp_CMD_c(upTrials(97:111)) = data;
data = outlier_t(LFvp_CMD(downTrials(97:111)));
LFvp_CMD_c(downTrials(97:111)) = data;
clear data;

% transpose and calculate standardized variable
LFvp_CMD_c = LFvp_CMD_c';
bkup_mean = nanmean(LFvp_CMD_c(upTrials(11:20)));
bkup_std = nanstd(LFvp_CMD_c(upTrials(11:20)));
LFvp_CMD_up_st = (LFvp_CMD_c(upTrials) - bkup_mean)/bkup_std;

bkdown_mean = nanmean(LFvp_CMD_c(downTrials(11:20)));
bkdown_std = nanstd(LFvp_CMD_c(downTrials(11:20)));
LFvp_CMD_down_st = (LFvp_CMD_c(downTrials) - bkdown_mean)/bkdown_std;

clear bkup_mean; clear bkup_std; clear bkdown_mean; clear bkdown_std;
LFvp_CMD = LFvp_CMD';

%% Plotting Code for LFvp_CMD
figure
set(gcf,'Color','w','Position',[560 528 600 420])
hold on;

subplot('Position',[0.06 0.2 0.1 0.6]); hold on;
plot(upTrials(1:10),LFvp_CMD(upTrials(1:10)),'bo');
hold on
plot(upTrials(1:10),LFvp_CMD_c(upTrials(1:10)),'bx');
hold on
plot(downTrials(1:10),LFvp_CMD(downTrials(1:10)),'ro');
hold on
plot(downTrials(1:10),LFvp_CMD_c(downTrials(1:10)),'rx');
axis([0 20 -3 3]); set(gca,'LineWidth',2,'XTick',[1 10 20],'YTick',-3:0.5:3,'YTickLabel', -3:0.5:3,'FontName','Arial','FontSize',10); ylabel('LFvp_CMD [N]'); title('vis-pre','fontsize',11);

hold on
subplot('Position',[0.19 0.2 0.1 0.6]); hold on;
plot(upTrials(11:20)-kbTrials(1)+1,LFvp_CMD(upTrials(11:20)),'bo');
hold on
plot(upTrials(11:20)-kbTrials(1)+1,LFvp_CMD_c(upTrials(11:20)),'bx');
hold on
plot(downTrials(11:20)-kbTrials(1)+1,LFvp_CMD(downTrials(11:20)),'ro');
hold on
plot(downTrials(11:20)-kbTrials(1)+1,LFvp_CMD_c(downTrials(11:20)),'rx');
axis([0 20 -3 3]); set(gca,'LineWidth',2,'XTick',[1 10 20],'YTick',[],'YTickLabel',[],'FontName','Arial','FontSize',10); title('kin-pre','fontsize',11);

hold on
subplot('Position',[0.32 0.2 0.4 0.6]); hold on;
plot(upTrials(22:91)-exTrials(1)+1,LFvp_CMD(upTrials(22:91)),'bo');
hold on
plot(upTrials(22:91)-exTrials(1)+1,LFvp_CMD_c(upTrials(22:91)),'bx');
hold on
plot(downTrials(22:91)-exTrials(1)+1,LFvp_CMD(downTrials(22:91)),'ro');
hold on
plot(downTrials(22:91)-exTrials(1)+1,LFvp_CMD_c(downTrials(22:91)),'rx');
axis([0 140 -3 3]); set(gca,'LineWidth',2,'XTick',[1 20 70 120 140],'YTick',[],'YTickLabel',[],'FontName','Arial','FontSize',10); title(['exposure'], 'fontsize',11); xlabel('Trials','fontsize',11);

hold on
subplot('Position',[0.75 0.2 0.24 0.6]); hold on;
plot(upTrials(92:111)-peTrials(1)+1,LFvp_CMD(upTrials(92:111)),'bo');
hold on
plot(upTrials(92:111)-peTrials(1)+1,LFvp_CMD_c(upTrials(92:111)),'bx');
hold on
plot(downTrials(92:111)-peTrials(1)+1,LFvp_CMD(downTrials(92:111)),'ro');
hold on
plot(downTrials(92:111)-peTrials(1)+1,LFvp_CMD_c(downTrials(92:111)),'rx');
axis([0 40 -3 3]); set(gca,'LineWidth',2,'XTick',[1 20 40],'YTick',[],'YTickLabel',[],'FontName','Arial','FontSize',10); title(['post-exposure'], 'fontsize',11); xlabel('Trials','fontsize',11);
title([subID(7:9), ' ', 'LFvp_CMD'])
...
axis([-23.5 6.5 -15 15]); set(gca,'LineWidth',2,'XTick',[-23.5 -8.5 6.5],'YTick',[-15 -10 0 10 15],'YTickLabel',[-15 -10 0 10 15],'FontName','Arial','FontSize',10); title([subID(7:9),' ','Late Post-Exposure'],'fontsize',11);
axis square
hold on
plot(sortData(1).TARGET_TABLE.X(2)*(-1)+r*cos(ang),sortData(1).TARGET_TABLE.Y(2)+r*sin(ang),'Color',[255/255 117/255 56/255]) %home position
hold on
...
% %switch Directory
% if strcmp(str,'MACI64') == 1
%     cd(['/Volumes/mnl/Data/Adaptation/interference_dosing/Post_Step_2']);
% else
%     %cd(['Z:\Data\Adaptation\interference_dosing\Post_Step_2']); % Lab PCs
%     cd(['C:\Users\Alex\Desktop\IFDosing\Post_Step_2']); % Home PC
% end
% 
% save([subID '_postStep2_lh' '.mat'],'sortData','downTrials','upTrials','fname','onset','offset','wrong_trial','channel_trial','EP_X', 'EP_X_c', 'EP_X_down_st', 'EP_X_up_st',...
...
%     'LFoff_FS', 'LFoff_FS_c', 'LFoff_FS_down_st', 'LFoff_FS_up_st',...
%     'LFoff_CMD', 'LFoff_CMD_c', 'LFoff_CMD_down_st', 'LFoff_CMD_up_st',...
%     'LFvp_FS', 'LFvp_FS_c', 'LFvp_FS_down_st', 'LFvp_FS_up_st',...
%     'LFvp_CMD', 'LFvp_CMD_c', 'LFvp_CMD_down_st', 'LFvp_CMD_up_st')