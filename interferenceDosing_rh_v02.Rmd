...
rhData_blocks = subset(rhData_blocks, select = -c(Group.1, Group.2, subjectID))
...
rhData_mbg = subset(rhData_mbg, select = -c(Group.1, Group.2, target_theta, trial))
rhData_mbg = arrange.vars(rhData_mbg, c("block"=2))

# SEM calculated by block and group for kinematic variables
rhData_sembg = aggregate(rhData_blocks, list(rhData_blocks$group, rhData_blocks$trial),
                         FUN = std.error, na.rm = TRUE)
rhData_sembg$group = rhData_sembg$Group.1
rhData_sembg = rhData_sembg[order(rhData_sembg$group),]
rhData_sembg = subset(rhData_sembg, select = -c(Group.1, Group.2, trial, target_theta, group))
...
## Plot Raw Data
```{r Plot raw data}
Data = subset(rhData, rhData$subjectID == 101)
...
```{r Plot EP_X}
ggplot(data = rhData_mbg, aes(x = block, y = EP_X))+
  geom_point(aes(color = rhData_mbg$group))
  #geom_smooth(aes(color = rhData_mbg$group))
```
...
ggplot(data = subset(rhData_MandSEMbg, rhData_MandSEMbg$phase %in% 1:4), aes(x = block, y = ide_st))+
  geom_point(aes(color = group_ex), position = pd)+
  geom_errorbar(data = subset(rhData_MandSEMbg, rhData_MandSEMbg$phase %in% 1:4),
                aes(x = block, ymin = ide_st-ide_st_sem, ymax = ide_st+ide_st_sem, color = group),
                position = pd, width = 0)+
